        ...
        <TouchableOpacity
            ...
            <View style={[ tw`flex-1 flex-row items-start justify-between ml-3` ]}>
                <View style={[ tw`flex-1 border-0 border-white` ]}>
                    <Text style={[ tw`text-black text-base font-medium`, {fontFamily:'Itim-Regular'} ]} numberOfLines={1} ellipsizeMode='tail'>{ item.conducteur ? (item.conducteur.nom + ' ' + item.conducteur.prenom) : 'Waataxi Service' }</Text>
                    <View style={[ tw`` ]}>
                    ...
                    <Text style={[ tw`text-sm pl-1`, {color: ColorsEncr.main} ]} numberOfLines={1} ellipsizeMode='tail'>{reservation?getLocalDate(item.date_depart):getLocalDate(item.dat)} {reservation?item.heure_depart.slice(0, 5):getLocalTime(item.dat)}</Text>
